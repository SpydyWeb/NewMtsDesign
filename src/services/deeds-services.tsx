...
class DeedsServices {
  async getAll(orderId: number, pageNo: number, pageSize: number, sortColumn: string, sortDirection: string) {
    ...
    return axios.post(
      import.meta.env.VITE_DEED_API_URL, 
        {"eventType": "order_property_deed_get_page",
          "payload": {
            "orderId": orderId,
            "pageNo": pageNo,
            "pageSize": pageSize,
  ...
  async save(deed: Deed){
    const user  = await Auth.currentAuthenticatedUser();
    const token = user.signInUserSession.idToken.jwtToken;

    return axios.post(
      import.meta.env.VITE_DEED_API_URL, 
        {"eventType": "order_property_deed_save",
          "payload": deed}, {
            headers: {
              Authorization: token
            }
  ...
  async deleteItem(id: number){
    ...
    return axios.post(
      import.meta.env.VITE_DEED_API_URL, 
        {"eventType": "order_property_deed_delete",
          "payload": {
            "id": id
          }}, {
            headers: {
  ...
  async getSize(orderId: number){
    ...
    return axios.post(
      import.meta.env.VITE_DEED_API_URL, 
        {"eventType": "order_property_deed_get_size",
          "payload": {
            "orderId": orderId,
          }}, {
            headers: {
