...
class MortgageServices {
  ...
  async getAll(orderId: number, pageNo: number, pageSize: number, sortColumn: string, sortDirection: string) {
    ...
    return axios.post(
      import.meta.env.VITE_MORTGAGE_API_URL, 
        {"eventType": "order_property_mortgage_get_page",
          "payload": {
            "orderId": orderId,
            "pageNo": pageNo,
            "pageSize": pageSize,
  ...
  async save(mortgage: Mortgage){
    const user  = await Auth.currentAuthenticatedUser();
    const token = user.signInUserSession.idToken.jwtToken;

    return axios.post(
      import.meta.env.VITE_MORTGAGE_API_URL, 
        {"eventType": "order_property_mortgage_save",
          "payload": mortgage}, {
            headers: {
              Authorization: token
            }
  ...
  async deleteItem(id: number){
    ...
    return axios.post(
      import.meta.env.VITE_MORTGAGE_API_URL, 
        {"eventType": "order_property_mortgage_delete",
          "payload": {
            "id": id
          }}, {
            headers: {
  ...
  async getSize(orderId: number){
    ...
    return axios.post(
      import.meta.env.VITE_MORTGAGE_API_URL, 
        {"eventType": "order_property_mortgage_get_size",
          "payload": {
            "orderId": orderId,
          }}, {
            headers: {
