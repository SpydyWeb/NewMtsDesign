...
export const TabLink = styled.li<{ active: boolean }>`
  ...
  background: ${({ active }) =>
    active
      ? "-moz-linear-gradient(45deg, " +
        pallete.tableBackgroundColor +
        " 30%, " +
        pallete.additionalTableBackgroundColor +
        " 70%)"
      : " "};
  background: ${({ active }) =>
    active
  ...
  filter: ${({ active }) =>
    active
      ? "progid:DXImageTransform.Microsoft.gradient( startColorstr='#05abe0', endColorstr='#8200f4',GradientType=1 )"
      : " "};
  transition-duration: ${({ active }) =>
    active ? "all 0.6s ease-in-out" : " "};
  transition-timing-function: ${({ active }) =>
    active ? "cubic-bezier(0.68, -0.55, 0.265, 1.55)" : " "};
  font-weight: ${({ active }) => (active ? "500" : "none")};

  &:hover {
    background: -moz-linear-gradient(
      45deg,
      ${pallete.additionalTableBackgroundColor} 30%,
      ${pallete.tableBackgroundColor} 70%
    );
