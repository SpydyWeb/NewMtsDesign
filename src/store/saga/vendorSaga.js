...
import { GetStateList, GetallVendorBySearch, GetLicenceType, GetCommunicationTypeList, AddLicenceType, AddCommunication, DeleteLicenceType, DeleteCommuncationType, UpdateLicenceType, UpdateCommuncationType, AddState, UpdateState, DeleteState, GetVendorProduct, } from "../../servicesapi/Vendorapi";
let data = [];
...
function* addcommunicationtypedata(formdata) {
    ...
    if (formdata.data.editid === undefined || formdata.data.editid === "")
        ...
        res = yield UpdateCommuncationType(formdata.data.formData, formdata.data.editid);
    if (res.status === 200) {
        yield put({ type: constant.SET_LOADING, value: "save" });
...
function* addstatedata(formdata) {
    ...
    if (formdata.data.editid === undefined || formdata.data.editid === "")
        ...
        res = yield UpdateState(formdata.data.formData, formdata.data.editid);
    if (res.status === 200) {
        yield put({ type: constant.SET_LOADING, value: "save" });
        yield put({ type: constant.SET_DIALOGUE_VIEW, value: "" });
        yield getstatedata();
